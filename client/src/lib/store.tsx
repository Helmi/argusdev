...
export function AppProvider({children}: {children: ReactNode}) {
	...
	const [contextSidebarSessionId, setContextSidebarSessionId] = useState<
		string | null
	>(null);

	// Track which sessions have sidebar open (per-session preference)
	const [sessionsWithSidebarOpen, setSessionsWithSidebarOpenState] = useState<
		Set<string>
	>(() => {
		try {
	...
	const setSessionsWithSidebarOpen = useCallback(
	...
	const [sessionContextTabs, setSessionContextTabsState] = useState<
	...
	const setSessionContextTab = useCallback(
	...
	const [addWorktreeProjectPath, setAddWorktreeProjectPath] = useState<
		string | null
	>(null);
	const [addSessionOpen, setAddSessionOpen] = useState(false);
	const [addSessionWorktreePath, setAddSessionWorktreePath] = useState<
		string | null
	>(null);
	const [addSessionProjectPath, setAddSessionProjectPath] = useState<
		string | null
	>(null);
	const [addSessionTdTaskId, setAddSessionTdTaskId] = useState<string | null>(
		null,
	);
	const [addSessionIntent, setAddSessionIntent] =
		useState<AddSessionIntent | null>(null);
	const [addSessionSessionName, setAddSessionSessionName] = useState<
		string | null
	>(null);
	const [viewingFileDiff, setViewingFileDiff] = useState<{
		sessionId: string;
		file: ChangedFile;
		worktreePath: string;
	} | null>(null);
	...
	const [conversationInitialSessionId, setConversationInitialSessionId] =
	...
	const normalizeTdStatus = useCallback((data: unknown): TdStatus | null => {
		...
		return {
			availability: {
				...
				version:
					typeof availabilitySource.version === 'string'
						? availabilitySource.version
						: null,
				binaryPath:
					typeof availabilitySource.binaryPath === 'string'
						? availabilitySource.binaryPath
						: null,
			},
	...
	const fetchTdPrompts = useCallback(
		async (
	...
	const saveTdPrompt = useCallback(
		async (
			name: string,
			content: string,
	...
	const deleteTdPrompt = useCallback(
		async (
			name: string,
			scope: 'project' | 'global' = 'project',
		): Promise<boolean> => {
	...
	const closeTaskBoard = useCallback(() => {
		...
		if (
			...
			sessions.some(s => s.id === taskBoardReturnSessionId)
		) {
	...
	useEffect(() => {
		if (sessions.length > 0) {
			...
			const staleSidebarIds = [...sessionsWithSidebarOpen].filter(
			...
			const staleTabIds = Object.keys(sessionContextTabs).filter(
				id => !sessionIds.has(id),
			);
