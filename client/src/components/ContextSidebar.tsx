...
export function ContextSidebar() {
	...
	const unmergedWorktrees: Worktree[] = isNonWorktreeSession
		? (() => {
				...
				const sessionProject = projects.find(p =>
					session && session.path.startsWith(p.path),
				);
				...
				return worktrees.filter(w => {
					...
					if (w.path.startsWith(sessionProject.path)) return true;
	...
	const sessionProject = isNonWorktreeSession
		? projects.find(p => session && session.path.startsWith(p.path))
		: null;
